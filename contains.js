/**
 ...
 At each key we can check to see if the value of that key is itself an object such as the key stuff in our example, 
 its value is an object, we can use recursion to dig deeper into these objects until we find keys whose values are
 not nested objects.

 When the value of the key is not an object we can check to see if it is equal to the value provided. 
 If it is we can immediately return true since we found the value. If we loop through all the keys and do not find
 a match we return false.
